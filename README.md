Для запуска, после копирования репозитория:

В папке с проектом запустить npm install или yarn, а затем npm start или yarn start.

Для сборки в готовый билд: npm run build

Впрочем подробности описаны ниже:)
